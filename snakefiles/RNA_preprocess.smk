...
rule all:
    input:
        ...
        # Trimming outputs
        #[expand('rna_output/trim/{sampleName}_R1{ext}', sampleName=key, ext=['.fastq.gz_trimming_report.txt', '_val_1.fq.gz']) for key in read1],
        #[expand('rna_output/trim/{sampleName}_R2{ext}', sampleName=key, ext=['.fastq.gz_trimming_report.txt', '_val_2.fq.gz']) for key in read1],
        ...
        [expand('rna_output/quant/{sampleName}',
         ...
         for key in read1],
        #addreadgroup
        [expand('rna_output/align/{sampleName}_RG.{ext}',
            sampleName=key,
            ext=['bam','bam.bai']) for key in read1]
...
rule quant:
    ...
    shell:
        ...
        salmon quant -i {params.index} -l A -1 {input.trim1} -2 {input.trim2} -o rna_output/quant/{wildcards.sampleName} --seqBias --gcBias --validateMappings 1> {log.out} 2> {log.err}
        """
