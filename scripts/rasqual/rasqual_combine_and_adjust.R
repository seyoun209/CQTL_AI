...
# Script to process RASQUAL 1kb files and adjust p-values for tied SNPs
library(data.table)

# Define parameters
base_dir <- "/work/users/s/e/seyoun/CQTL_AI"  # Adjust this to your base directory
input_dir <- file.path(base_dir, "output/rasqual_qtl/results")
output_dir <- file.path(base_dir, "output/rasqual_qtl/results/combined_1kb")
...
# Get all 1kb RASQUAL result files
result_files <- list.files(
  path = input_dir, 
  pattern = "_RASQUALResults_1kb.txt$",
  recursive = TRUE,
  full.names = TRUE
)

cat("Found", length(result_files), "1kb RASQUAL result files\n")
...
for (i in 1:nrow(condition_pc_combos)) {
  ...
  combined_file <- file.path(output_dir,
                             paste0(current_condition, "_", current_pc, "_1kb_combined.txt"))
  ...
  for (j in 1:length(features)) {
    ...
    # First sort by p-value and distance from peak center
  ...
  adjusted_file <- file.path(output_dir,
                             paste0(current_condition, "_", current_pc, "_1kb_adjusted.txt"))
